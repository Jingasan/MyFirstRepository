...
int ExhaustiveSearch(
	...
	/* XYZ 各方向の探索空間サイズ */
	cout << " X 方向の探索空間サイズ : " << ( upperX - lowerX ) << endl;
	cout << " Y 方向の探索空間サイズ : " << ( upperY - lowerY ) << endl;
	cout << " Z 方向の探索空間サイズ : " << ( upperZ - lowerZ ) << endl;
	...
	double voxelSize = 2 * targetInformation->max / DistanceFieldDevideSize;	// DF のボクセルサイズ
	...
	/* 全探索後の RMSE 評価値 */
	double RMSE_ExhaustiveSaerch = ( voxelSize / 1000 ) * sqrt( distanceMeans );
	...
	cout << "Degree: " << minDegree( 0 ) << ", " << minDegree( 1 ) << ", " << minDegree( 2 ) << endl;
	cout << "DistanceMeans: " << distanceMeans << endl;
	cout << "全探索後RMSE: " << RMSE_ExhaustiveSaerch << endl << endl;


	(*ER).push_back( minR );
	(*Et).push_back( mint );
	(*DFval).push_back( *minSumofDistance );
	...
	/*** 全探索結果の出力 ***/
	FILE *outputFp1;
	if( ( outputFp1 = fopen( ExhaustiveSearchParameterFileName, "w" ) ) == NULL ){ cout << "Caution!: " << ExhaustiveSearchParameterFileName << " open error" << endl; return 1; }
	fprintf( outputFp1, "R,,,t,s,b\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n",
		(*ER)[ 0 ]( 0, 0 ), (*ER)[ 0 ]( 0, 1 ), (*ER)[ 0 ]( 0, 2 ), (*Et)[ 0 ]( 0 ),
		(*ER)[ 0 ]( 1, 0 ), (*ER)[ 0 ]( 1, 1 ), (*ER)[ 0 ]( 1, 2 ), (*Et)[ 0 ]( 1 ),
		(*ER)[ 0 ]( 2, 0 ), (*ER)[ 0 ]( 2, 1 ), (*ER)[ 0 ]( 2, 2 ), (*Et)[ 0 ]( 2 ),
		0, 0, 0, 0
	);
	fclose( outputFp1 );
	
	/*** 全探索結果の探索パラメータを用いた結果の出力 ***/
	...
	/* 出力ファイルパスの設定 */
	output1.open( CPDistanceAfterESFileName );
	output2.open( ExhaustiveSearchRMSEFileName );

	/* モデル点群の各点から探索対象点群中の最近点までの距離の算出 */
	for( int i = 0; i < transformedPointCloud->points.size(); i++ ){

		kdtree.nearestKSearch( transformedPointCloud->points[ i ], 1, index, distance2 );
	...
	/* 最近点までの距離の平均値 */
	meanDistance = sumOfDistance / transformedPointCloud->points.size();
	esRMSE = sqrt( sumOfDistance2 / transformedPointCloud->points.size() );
	cout << "mean: " << meanDistance << " [voxel]" << endl;
	cout << "RMSE: " << esRMSE << " [voxel]" << endl;

	/* RMSE と 対応点間の距離の平均値の出力 */
	output2 << "mean," << meanDistance << ",[voxel]" << endl;
	output2 << "RMSE," << esRMSE << ",[voxel]" << endl;
