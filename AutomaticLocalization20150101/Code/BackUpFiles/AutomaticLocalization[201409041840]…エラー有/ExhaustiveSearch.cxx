...
int ExhaustiveSearch(
	...
	Eigen::Matrix< double, 3, 3 > **ER,				// 全探索結果の回転行列 R
	Eigen::Vector3d **Et,							// 全探索結果の並進ベクトル t
	int** EDFval									// 全探索結果のDF評価値の合計値：小さいものから順に格納
){
	
	/*** 変数の宣言 ***/
	int i, j, r, n, tx, ty, tz;	// カウンタ変数
	...
	double element0;	// 四元数の1つ目の要素
	double element1;	// 四元数の2つ目の要素 
	double element2;	// 四元数の3つ目の要素
	double element3;	// 四元数の4つ目の要素
	double element0S;	// 四元数の1つ目の要素の2乗
	double element1S;	// 四元数の2つ目の要素の2乗
	double element2S;	// 四元数の3つ目の要素の2乗
	double element3S;	// 四元数の4つ目の要素の2乗
	...
	int breakFlag = 0;			// 全探索計算の打ち切り用フラグ
	int startFlag = 1;			// 全探索の初めのみを示すフラグ
	...
	int tempEDFval;							// 全探索結果のDF評価値の合計値の一時格納用変数
	Eigen::Matrix< double, 3, 3 > tempER;	// 全探索結果の回転行列 R の一時格納用変数
	Eigen::Vector3d tempEt;					// 全探索結果の並進ベクトル t の一時格納用変数
	...
	/* 指定した数だけ全探索結果の R, t, DF の合計評価値を確保 */
	( *ER ) = new Eigen::Matrix< double, 3, 3 >[ CANDIDATE_NUM ];	// 全探索結果の回転行列 R
	( *Et ) = new Eigen::Vector3d[ CANDIDATE_NUM ];					// 全探索結果の並進ベクトル t
	( *EDFval ) = new int[ CANDIDATE_NUM ];							// 全探索結果のDF評価値の合計値：小さいものから順に格納
	
	/* 回転の均等なサンプリング */
	quaternions.resize( 0 );	// 2次元配列 … 複数個の四元数
	quaternion.resize( 4 );		// 1次元配列 … 四元数
	degrees.resize( 0 );		// 2次元配列 … 複数個の θ, Φ, Ψ
	degree.resize( 3 );			// 1次元配列 … 四元数に対応する θ, Φ, Ψ
	...
	/* 全探索結果のDF評価値の合計値の初期化 */
	for( i = 0; i < CANDIDATE_NUM; i++ ){ ( *EDFval )[ i ] = INT_MAX; }	// DF評価値の合計値を小さいものから順に格納


	/*** モデル点群の重心の算出 ***/

	/* 点群の端点を保存 */
	for( i = 0; i < cloudM->points.size(); i++ ){
...
#ifdef USEOPENMP
#pragma omp parallel //private( element0, element1, element2, element3, element0S, element1S, element2S, element3S )//, *R, **quaternions )
{
#pragma omp for private( element0, element1, element2, element3, element0S, element1S, element2S, element3S )
#endif

	for( i = 0; i < quaternions.size(); i++ ){

		element0 = (double)quaternions[ i ][ 0 ];	// 四元数の1つ目の要素
		element1 = (double)quaternions[ i ][ 1 ];	// 四元数の2つ目の要素 
		element2 = (double)quaternions[ i ][ 2 ];	// 四元数の3つ目の要素
		element3 = (double)quaternions[ i ][ 3 ];	// 四元数の4つ目の要素
	...
	cout << "Time: " << (double)( TimeEnd - TimeStart ) << endl;
	...
	/* モデル点群の回転 */
	for( r = 0; r < quaternions.size(); r++ ){

		cout << "RotationID: " << r << endl << "Degree: ";
		for( i = 0; i < degrees[ r ].size(); i++ ){ cout << degrees[ r ][ i ] << " "; }
		cout << endl << "R: " << endl << R[ r ] << endl;

		/* DF の座標系への変換, 回転後のモデル点群 */
		for( i = 0; i < cloudM->size(); i++ ){
		...
		/* モデル点群の並行移動 */
		for( tz = lowerZ; tz < upperZ; tz += sampZ ){
			for( ty = lowerY; ty < upperY; ty += sampY ){
				for( tx = lowerX; tx < upperX; tx += sampX ){
					//tx = 0; ty = 0; tz = 0;
					...
					for( i = 0; i < rotatedPointCloud->size(); i++ ){
					...
					sum = 0;	// 初期化
					for( n = 0; n < transPointCloud->size(); n++ ){
						...
						if( sum > ( INT_MAX / 10 ) || sum > ( *EDFval )[ CANDIDATE_NUM - 1 ] ){ breakFlag = 1; break; }
#else
						/* 最小合計距離評価値よりも現在の合計距離評価値の方が大きい場合 */
						if( sum > *minSumofDistance || sum > ( INT_MAX / 10 ) ){ breakFlag == 1; break; }
#endif
						/* 指定した1点あたりのDF距離評価値 * 点群数よりも現在の合計距離評価値の方が大きい場合 */
						if( sum > POINT_THRESHOLD * transPointCloud->size() ){ breakFlag = 1; break; }
					}
...
#ifdef ES_CANDIDATE
					/*** 全探索結果の回転行列 R と並進ベクトル t を DF の合計評価値が小さいものから順にソーティング ***/
					if( SumofDistance < ( *EDFval )[ CANDIDATE_NUM - 1 ] && breakFlag == 0 ){
						
						( *EDFval )[ CANDIDATE_NUM - 1 ] = SumofDistance;
						( *ER )[ CANDIDATE_NUM - 1 ] = R[ r ];
						( *Et )[ CANDIDATE_NUM - 1 ] = t;

						for( j = CANDIDATE_NUM - 1; j > 0; j-- ){
							if( ( *EDFval )[ j ] < ( *EDFval )[ j - 1 ] ){
								tempEDFval = ( *EDFval )[ j ]; ( *EDFval )[ j ] = ( *EDFval )[ j - 1 ]; ( *EDFval )[ j - 1 ] = tempEDFval;
								tempER = ( *ER )[ j ]; ( *ER )[ j ] = ( *ER )[ j - 1 ]; ( *ER )[ j - 1 ] = tempER;
								tempEt = ( *Et )[ j ]; ( *Et )[ j ] = ( *Et )[ j - 1 ]; ( *Et )[ j - 1 ] = tempEt;
							}else{
								break;
							}
						}
					...
					if( SumofDistance < *minSumofDistance && breakFlag == 0 ){
					
						*minSumofDistance = SumofDistance; 

						/* 最小距離評価値をとる回転行列と並進ベクトル */
...
#ifdef COMMENT
#ifdef ES_CANDIDATE
		cout << "t: " << ( *Et )[ 0 ]( 0 ) << ", " << ( *Et )[ 0 ]( 1 ) << ", " << ( *Et )[ 0 ]( 2 ) << endl;
		cout << "R: " << ( *ER )[ 0 ]( 0, 0 ) << "\t" << ( *ER )[ 0 ]( 0, 1 ) << "\t" << ( *ER )[ 0 ]( 0, 2 ) << endl;
		cout << "   " << ( *ER )[ 0 ]( 1, 0 ) << "\t" << ( *ER )[ 0 ]( 1, 1 ) << "\t" << ( *ER )[ 0 ]( 1, 2 ) << endl;
		cout << "   " << ( *ER )[ 0 ]( 2, 0 ) << "\t" << ( *ER )[ 0 ]( 2, 1 ) << "\t" << ( *ER )[ 0 ]( 2, 2 ) << endl;
		cout << "DF合計評価値：" << ( *EDFval )[ 0 ] << endl;
#else
		cout << "最小合計距離評価値: " << *minSumofDistance << endl;
		cout << "t: " << mint( 0 ) << ", " << mint( 1 ) << ", " << mint( 2 ) << endl;
	...
	for( i = 0; i < CANDIDATE_NUM; i++ ){
		...
		cout << "t: " << ( *Et )[ i ]( 0 ) << ", " << ( *Et )[ i ]( 1 ) << ", " << ( *Et )[ i ]( 2 ) << endl;
		cout << "R: " << ( *ER )[ i ]( 0, 0 ) << "\t" << ( *ER )[ i ]( 0, 1 ) << "\t" << ( *ER )[ i ]( 0, 2 ) << endl;
		cout << "   " << ( *ER )[ i ]( 1, 0 ) << "\t" << ( *ER )[ i ]( 1, 1 ) << "\t" << ( *ER )[ i ]( 1, 2 ) << endl;
		cout << "   " << ( *ER )[ i ]( 2, 0 ) << "\t" << ( *ER )[ i ]( 2, 1 ) << "\t" << ( *ER )[ i ]( 2, 2 ) << endl;
		cout << "DF合計評価値：" << ( *EDFval )[ i ] << endl;
		cout << "1点あたりのDF評価値" << (double)( *EDFval )[ i ] / (double)cloudM->size() << endl << endl;
	}
	...
	cout << "   " << minR( 2, 0 ) << "\t" << minR( 2, 1 ) << "\t" << minR( 2, 2 ) << endl;
	cout << "Degree: " << minDegree( 0 ) << ", " << minDegree( 1 ) << ", " << minDegree( 2 ) << endl;
	cout << "1点あたりのDF評価値: " << distanceMeans << endl << endl;
