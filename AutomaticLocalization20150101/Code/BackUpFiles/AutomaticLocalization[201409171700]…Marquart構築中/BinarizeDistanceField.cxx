/***** ディスタンスフィールドの二値初期化 ( CreateDistanceFieldforPCDModel 関数内で使用 ) *****/
...
void BinarizationDFforPCDModel(
	...
	int **ClosestPointID,						// 最近点 ID
	struct ModelPointCloud** ModelPointCloud,	// 点群( DF の座標系に直したもの, 単位:mm )
	int *PointSize,								// 点群数
	struct ModelInformation *ModelInf,			// 点群の各種情報
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,	// 入力点群
	double gravX,								// 点群の X 方向の重心
	double gravY,								// 点群の Y 方向の重心
	double gravZ								// 点群の Z 方向の重心
